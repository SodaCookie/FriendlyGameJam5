...
public class TestMelodrive
{
    ...
    private IEnumerator TestEnsembles()
    {
        ...
        int num = md.GetNumEnsembles();
        Assert.Greater(md.GetNumEnsembles(), 1, "There should be at least two Ensembles");

        //Test Ensemble change
        string ensemble = md.GetCurrentEnsemble();
        string oldEnsemble = ensemble;

        int i = Random.Range(0, num-1);
        while (md.GetEnsembleID(i) == ensemble)
            i = Random.Range(0, num-1);
        ensemble = md.GetEnsembleID(i);
    ...
    public class MDBehaviourTest : MonoBehaviour
    {
        ...
        public Emotion initEmotion = Emotion.Neutral;
