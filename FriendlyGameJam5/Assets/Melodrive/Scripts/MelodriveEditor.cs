...
{
    /**
     * MelodriveEditor is a Scene that shows off all the features of Melodrive in one simple interface.
     * Here you can play round with Style and Emotion and save/load your seeds and projects.
     */
    public class MelodriveEditor : MelodriveObject
    {
        public Style style = Style.House;
        public Emotion emotion = Emotion.Tender;
        ...
        private Dictionary<int, string> ensembleValues;
        ...
        void Start()
        {
            ...
            foreach (Style s in Enum.GetValues(typeof(Style)))
            {
                ...
                item.text = Enum.GetName(typeof(Style), s);
                options.Add(item);
                if (style == s)
                    selectedValue = (int)s;
            }
            ...
            for (int i = 0; i < points.Length; i++)
            {
                ...
                if (p.name == Enum.GetName(typeof(Emotion), emotion))
                {
                    target = p;
                    initPos = pos;
                }
        ...
        {
            ensembleValues = new Dictionary<int, string>();
            Dropdown ensembleChange = GameObject.Find("EnsembleSelect").GetComponent<Dropdown>();
            ensembleChange.ClearOptions();
            Dictionary<string, string> ensembles = md.GetEnsembles();
            string currentEnsemble = md.GetCurrentEnsemble();
            int value = 0;
            int i = 0;
            List<Dropdown.OptionData> options = new List<Dropdown.OptionData>();
            foreach (KeyValuePair<string, string> kv in ensembles)
            {
                Dropdown.OptionData item = new Dropdown.OptionData();
                item.text = kv.Value;
                options.Add(item);
                if (kv.Key == currentEnsemble)
                    value = i;
                ensembleValues[i] = kv.Key;
                i += 1;
            }
        ...
        public void OnUIStyleChange()
        {
            ...
            if (md)
            {
                Style style = (Style)styleChange.value;
                Debug.Log("Style changed: " + Enum.GetName(typeof(Style), style));
                md.SetStyle(style);
            }
        }
        ...
        public void OnUIEnsembleChange()
        {
            ...
            if (md)
            {
                Debug.Log("Ensemble changed: " + ensembleValues[ensembleChange.value]);
                md.SetEnsemble(ensembleValues[ensembleChange.value]);
            }
        }
